...
Cuando se implementen pantallas en React, importar strings desde `src/copy/es.ts` — no hardcodear en
componentes.
...
Firebase Authentication debe soportar **correo/contraseña** y **Google Sign-In**. Google es el
acceso recomendado en landing y login.
...
Pantalla/modal bloqueante tras autenticación con Google cuando el usuario **no tiene username** en
Firestore. Ruta propuesta: `/completar-username`.
...
**Prototipo:** Login/Registro → clic **Continuar con Google** → `Google — Username` → **Continuar al
panel** → `Google — Username — Cargando`
